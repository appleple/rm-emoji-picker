...
var EmojiPicker = function () {
    ...
    _createClass(EmojiPicker, [{
        ...
        value: function openPicker(event) {
            ...
            var tooltip = new _rmTooltip2.default(this._icon, this._container, this.$picker.get(0));
            tooltip.center();
        ...
        key: "setActiveCategoryByName",
        value: function setActiveCategoryByName(name) {
            var cat = this.getCategory(name);
            console.log(cat);
            if (!cat) {
                throw new ReferenceError("Your call to setActiveCategoryByName in rm-emoji-picker was not supplied a valid name. Valid names are: " + this.categories.map(function (cat) {
                    return cat.title;
                }).join(", ").slice(0, -2));
            }
        ...
        value: function _setCategoryTooltips() {
            ...
            if (this.$picker && this.defaults.show_icon_tooltips) {
                ...
                $cats.off('mouseenter.emoji').on('mouseenter.emoji', /**@this {HTMLElement}*/function (event) {
                    ...
                    var title = this.getAttribute('data-name');
                    tooltip = new _rmTooltip2.default(this, _picker.$picker.get(0), (0, _icon_tooltip2.default)({
                        text: title
                    }));
